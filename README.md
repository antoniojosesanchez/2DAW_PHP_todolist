...
### Twig y la carpeta `vendor`

Twig se instala con **Composer** desde la carpeta `todolist/`. En este repositorio **no** se versiona `todolist/vendor`: cada quien genera las dependencias en local (o en el contenedor) y esa carpeta queda ignorada por Git.

1. Entra en `todolist` (donde está el `composer.json`).
2. Si quieres empezar de cero, **elimina** la carpeta `vendor` (`rm -rf vendor` en macOS/Linux, o bórrala a mano en el explorador).
3. Instala dependencias:
   - **Tras clonar o con `composer.lock` actual:** `composer install` (deja Twig en la versión fijada en el lock).
   - **Subir Twig a la última versión compatible con el proyecto** (`^3.0` en `composer.json`): `composer update twig/twig` (actualiza solo ese paquete y reescribe `composer.lock`).

Necesitas [Composer](https://getcomposer.org/) instalado. Si no lo tienes en el host pero sí Docker, puedes usar algo como:  
`docker run --rm -v "$(pwd)/todolist:/app" -w /app composer:latest install`

Después de `composer install` o `update`, vuelve a levantar o usar la app como siempre; sin `vendor`, PHP no encontrará Twig hasta que ejecutes uno de esos comandos.
